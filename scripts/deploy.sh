...
# - Распаковывает, ставит зависимости, запускает webhook-демона
...
stop_bot() {
    ...
    if [ ! -f "${PROJECT_DIR}/webhook-daemon.pid" ]; then
        log_msg "No PID file, daemon not running"
        return 0
    fi

    pid=$(cat "${PROJECT_DIR}/webhook-daemon.pid" 2>/dev/null)
    if [ -z "$pid" ] || ! kill -0 "$pid" 2>/dev/null; then
        log_msg "Stale PID file, cleaning up"
        rm -f "${PROJECT_DIR}/webhook-daemon.pid"
        return 0
    fi

    log_msg "Stopping daemon (PID $pid)..."
    cd "$PROJECT_DIR" || return 1
    "$NODE_BIN" src/entrypoints/webhook-daemon.js stop 2>&1
...
start_bot() {
    cd "$PROJECT_DIR" || return 1
    log_msg "Starting webhook-daemon..."
    "$NODE_BIN" src/entrypoints/webhook-daemon.js >> "$BOT_LOG" 2>&1 &
    sleep 2

    if [ -f "webhook-daemon.pid" ] && kill -0 "$(cat webhook-daemon.pid)" 2>/dev/null; then
        log_msg "Webhook-daemon started (PID $(cat webhook-daemon.pid))"
        return 0
    fi

    log_msg "ERROR: Webhook-daemon failed to start"
    return 1
}
...
if [ ! -f "$ENV_FILE" ]; then
    ...
    log_msg "Create it with TG_BOT_API_TOKEN, FIREBASE_SERVICE_ACCOUNT_JSON, WEBHOOK_URL, WEBHOOK_PORT, WEBHOOK_SECRET_TOKEN"
    exit 1
fi
